...
## Workflow - Protocol Synopsis (REQUIRED)
...
Step 4. Generate protocol synopsis sections by delegating to `synopsis_sections_agent` in parallel using `task()`. Spawn a **maximum of 4 synopsis_sections_agent subagents** at any given time. If any of the subagents didn't provide results, you MUST **retry a maximum of upto 2 times**. You are not allowed to generate any additional section than the below ones.
...
Step 6: **Save Synopsis**: Save the generated synopsis in tabular form to the filesystem using `write_file()` tool. Save the generated synopsis to path `/synopsis/protocol_synopsis.md`. If the file exists, overwrite the existing file. Add the study title to the protocol synopsis at the start. You MUST not add anything additional.
...
When the full protocol is generated, return only a success message.
